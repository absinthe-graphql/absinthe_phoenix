defmodule Absinthe.Phoenix.Controller do
  ...
  To remove the need for reparsing values, scalar values aren't serialized; Phoenix actions receive the original,
  unserialized values of GraphQL fields.

  This is especially useful for custom scalar types. Using a couple of the additional types packaged in `Absinthe.Type.Custom`,
  for example:
  ...
  Unlike in the GraphQL notation scheme we prefer for GraphQL APIs (that is, `camelCase` fields, which better match
  up with the expectations of JavaScript clients), fields used in documents provided as `@graphql` should use
  `snake_case` naming, as Elixir conventions use that notation style for atoms, etc.

  ### Atom keys

  Because you are writing the GraphQL document in your controller and Absinthe is validating
  the document against your schema, atom keys are returned for field names.
  """
  ...
  value. `cast_params/3` is overridable and the implementation already
  ...
  """
  @callback absinthe_pipeline(schema :: Absinthe.Schema.t, variables :: %{String.t => any}) :: Absinthe.Pipeline.t
